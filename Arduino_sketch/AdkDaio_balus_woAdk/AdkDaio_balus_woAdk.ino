...
AndroidAccessory acc("Dorobook,Socym",   //第1引数:組織名（manufacturer属性と一致）
    ...
    "http://accessories.android.com/",   //第5引数:ジャンプ先URLk
    "0000000012345678");                 //第6引数:シリアル番号

byte b0;//タクトスイッチのひとつ前の状態を保存する
byte adk_status = false;//ADK接続の有無を示す状態フラグ
...
void loop() //繰り返し実行される部分
{
  ...
  static byte msg[3];//Androidとやりとりするデータ

  //Androidとの接続処理(4)
  if (acc.isConnected()) {  //Androidを起動・接続する命令を送る
    //communicate with Android application
    if(adk_status == false){
      adk_status = true;//adk接続をした状態
      analogWrite(LED_PWM, 0); //念のため消しておく
    }
    int len = acc.read(msg, sizeof(msg), 3); // ADK接続から配列の先頭０番目読み込み
    if (len != -1){
      Serial.println(String(msg[2], HEX) + "len" + len);
    }
    if (0 < len) {
      ...
      switch(msg[0]){
        ...
        case 0xff://バルス・コマンド
          ...
          Serial.println(String(msg[1], HEX) + "yanzm");
      }
    }
    ...
    adk_status = false;//adk接続されていない状態
